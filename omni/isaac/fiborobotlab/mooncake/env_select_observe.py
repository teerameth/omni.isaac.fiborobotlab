...
class MoonCakeEnv(gym.Env):
    ...
    def __init__(
            ...
            max_episode_length=100*60,
            display_every_iter=999999,
            seed=0,
            headless=True,
            observ_selection = [1, 1, 1, 1, 1]    # robot_fall_angle, robot_velocity, robot_omega, ball_velocity, wheel_velocity
    ) -> None:
        from omni.isaac.kit import SimulationApp

        self._observ_selection = observ_selection
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(sliding_window_width, observ_selection[0]+(observ_selection[1]+observ_selection[2]+observ_selection[3]+observ_selection[4])*3,),
                                            dtype=np.float32)
    ...
    def step(self, action):
        ...
        if exceed_time_limit: print("RESET from time limit exceed.")
        if exceed_time_limit or robot_fall:
            ...
            total_time_step += time_step_count_before_reset
            f.write(str(total_time_step) + '\t' + str(time_step_count_before_reset) + '\n')
            f.close()
    ...
    def get_observations(self):
        if not self.headless and self._iteration_count % self._display_every_iter == 0:
            if (
                    self._my_world.current_time_step_index - self._steps_after_reset) * self._physics_dt / self._rendering_dt > self._render_counter:
        ...
        if reading.shape[0]:  # at least 1 data in buffer (use the most recent data)
            ...
            observations = []
            if self._observ_selection[0]: observations.append(robot_fall_angle)
            if self._observ_selection[1]: observations += list(robot_velocity)
            if self._observ_selection[2]: observations += list(robot_omega)
            if self._observ_selection[3]: observations += list(ball_velocity)
            if self._observ_selection[4]: observations += list(mooncake_wheel_velocities)
        ...
        ## Fill NaN with ZEROs
        while len(self._observations_buffer) < sliding_window_width: self._observations_buffer = [np.zeros((9),
                                                                                                           dtype=np.float32)] + self._observations_buffer
